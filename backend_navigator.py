...
from bs4 import BeautifulSoup  # for heuristic checks
...
def _openai_complete(
    ...
    Returns assistant content (string). Raises on error.
    """
...
class LLMNavigator:
    ...
    def __init__(self, max_hops: int = 5):
        ...
        print(f"Backend initialized - OpenAI model: {OPENAI_MODEL}")
    ...
    def get_llm_navigation_decision(
        self,
        page_content: str,
...
Decision (JSON only):"""
        ...
        try:
            raw = _openai_complete(sys_prompt, user_prompt, temperature=0.2, max_tokens=300)
    ...
    def has_job_listings(self, html: str, text: str) -> bool:
        ...
        job_cards = soup.find_all(
            ["div", "article", "li"],
            class_=re.compile(r"job|position|listing|opening|career", re.I),
        )
    ...
    def navigate_to_jobs(self, start_url: str, user_query: str) -> Tuple[Optional[str], List[Dict[str, str]]]:
        """
        ...
        while hop_count < self.max_hops:
            ...
            if self.has_job_listings(html, page_text):
                ...
                # Ensure we still return some links if the main extractor was empty
                if not links:
                    print("  ! Warning: Heuristic found jobs but no links extracted; retrying basic extraction")
                    soup = BeautifulSoup(html, "lxml")
                    tmp_links: List[Dict[str, str]] = []
                    for a in soup.find_all("a", href=True, limit=50):
                        href = a.get("href", "").strip()
                        text = a.get_text(strip=True)[:100]
                        if href and text:
                            full_url = urljoin(current_url, href)
                            tmp_links.append({"text": text, "url": full_url})
                    links = tmp_links
            ...
            reasoning = (decision or {}).get("reasoning", "No reason provided")
            ...
            if action == "found_jobs":
                ...
                next_url = (decision or {}).get("url", "").strip()
...
async def health():
    ...
    return {"status": "healthy", "service": "Internship Navigator", "model": OPENAI_MODEL}
...
async def navigate(request: NavigationRequest):
    ...
    try:
        ...
        final_url, found_links = navigator.navigate_to_jobs(
            request.start_url, request.query
        )
...
async def root():
    ...
    return {
        ...
        "endpoints": {"health": "GET /health", "navigate": "POST /navigate", "fetch": "POST /fetch"},
    }
