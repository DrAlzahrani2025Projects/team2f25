"""
scraper.py — CSUSB internships page scraper with simple enrichment.

What it does
- Loads https://www.csusb.edu/cse/internships-careers with Playwright
- Extracts relevant anchors from the main content
- Normalizes absolute links
- Infers `company` from the link's domain (e.g., careers.microsoft.com -> Microsoft)
- Guesses `location` from anchor text if present (very lightweight)
"""
...
EXCLUDE_KEYWORDS = ["form", "proposal", "evaluation", "pdf"]  # filter out admin/docs

CITY_HINTS = ["San Bernardino", "Riverside", "Los Angeles", "San Diego", "Irvine",
...
def _infer_company_from_url(url: str) -> str | None:
    ...
    try:
        ...
        if len(parts) >= 2:
            brand = parts[-2]  # microsoft, apple, nasa, oracle
        else:
            brand = parts[0]
...
def _guess_location_from_text(text: str) -> str | None:
    # super-light heuristic: look for known city/state tokens in the anchor text
...
def scrape_csusb_listings(url: str = CSUSB_CSE_URL, timeout_ms: int = 60000) -> pd.DataFrame:
